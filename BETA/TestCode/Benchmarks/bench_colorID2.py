...
def f1(requested_color):
    min_dif = 16581375  # Maximum possible difference of (255)^3
    closest_color = None
    for color in colors_dict.keys():
        known_color = np.array(tuple(map(int, color.replace(' ', '').strip('[]').split(','))))
        dif_val = np.sum((known_color - requested_color) ** 2)
        if min_dif > dif_val:
            min_dif = dif_val
...
fn1 = timeit.Timer(stmt='f1([240, 135, 10])', setup='from __main__ import f1')
fn2 = timeit.Timer(stmt='f2([240, 135, 10])', setup='from __main__ import f2')
