...
from BETA.dev02.models.SpchRecg import internet, google_sr
from BETA.dev02.models.ColorID import avg_color
from BETA.dev02.models.Txt2Spch import t2s_say
...
def active_handler(event):
    ...
    if 'color' in str(s2t_string):
        _, frame = cap.read()
        threading.Thread(target=avg_color, args=(frame, size, frame_height, frame_width, q), daemon=True).start()
...
def key_press_handler(event):
    ...
    if event.key == controls_dict.get('color check')[0]:
        threading.Thread(target=avg_color, args=(cap.read()[1], size, frame_height, frame_width, q), daemon=True).start()
        threading.Thread(target=t2s_say, args=(q.get(), q), daemon=True).start()
