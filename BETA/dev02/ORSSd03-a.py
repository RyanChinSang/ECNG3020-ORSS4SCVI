...
from BETA.dev02.models.Spch2Txt import internet, google_sr
...
def key_press_handler(event):
    ...
    if event.key == controls_dict.get('save')[0]:
        ...
        try:
            ...
            Image.open(newss).resize((int(bbox.width*fig.dpi), int(bbox.height*fig.dpi)), Image.ANTIALIAS).save(newss)
            sslist.append(newss)
        except IndexError:
...
configure(frame=init_frame, dir='Screenshots', version='v0.2b')
fig = plt.gcf()
ax = plt.gca()
